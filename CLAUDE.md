...
- **Recursos descargables**: los zips viven en `public/downloads/` y se listan en `src/data/resources.ts` (títulos/descripciones `{ es, en }`, categoría, artículo relacionado). Provienen de los artefactos del Codex Lab — SIEMPRE sanitizar antes de empaquetar (placeholders en lugar de tenants/usuarios/keys reales) y verificar que ningún zip contenga el hostname real del tenant.
- **Servicios externos** en `src/data/services.ts` (Web3Forms para el formulario de contacto, Kit para el newsletter). Patrón de degradación: con la key vacía, `ContactForm` cae a mailto y `NewsletterSignup` no se renderiza — el sitio nunca se rompe por configuración pendiente.
- **Idioma**: el sitio es bilingüe (ES por defecto / EN). La UI se traduce vía `src/i18n/` (contexto `LanguageProvider`, hook `useLang()` que da `{ lang, t }`, diccionario en `translations.ts`); los textos del perfil son campos `{ es, en }` en `profile.ts`. Los artículos `.md` están solo en español — en EN se muestra un aviso (`t.blog.spanishOnly`). Toggle ES/EN en el Navbar, persiste en localStorage. Nunca hardcodear strings de UI en componentes: siempre `t.*`. El usuario habla español; responderle en español.
- **Tema dual (dark/light)**: los componentes usan SOLO tokens semánticos (`bg-canvas`, `bg-card`, `bg-raised`, `border-line`, `text-strong`, `text-body`, `text-muted`, `text-faint`, `text-accent-text`) — nunca colores fijos como `slate-*` o `ink-*`. Las variables `--c-*` cambian entre `:root` (claro) y `.dark` (oscuro) en `src/index.css`; el azul de marca `sap-blue` (#0070f2) es fijo. Toggle en Navbar vía `src/hooks/useTheme.ts` (persiste en localStorage, default dark, script anti-FOUC en index.html). Excepciones válidas de `text-white`: sobre botones azules sólidos y en el Lightbox (overlay negro).
...
Usar `--color-canvas` para el color del fondo (`bg-canvas`), nunca `--color-base`: colisiona con `text-base` (tamaño de fuente) y puede volver invisible el texto. Los contenedores compartidos usan `.site-shell`; las páginas `.page-section`; los artículos `.article-page` y `.article-body`. Validar escritorio y móvil al cambiar navegación o tipografía.
